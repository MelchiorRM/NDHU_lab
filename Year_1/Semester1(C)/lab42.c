...
void sum(DeciVar a, DeciVar b, DeciVar *s)
{
	...
	while(l[0] > 0)
	{
		holder += (a.decimal[l[0] - 1] + b.decimal[l[0] - 1] - 2 * '0');
		s->decimal[l[0] - 1] = holder % 10 + '0';
		holder /= 10;
		l[0]--;
	}// trailling zero
	for (i = l_max - 1; i >= 0; i--)
	...
	while(pos > 0)
	{
		if (l[0] && l[1])
		{
			holder += (a.integer[l[0] - 1] + b.integer[l[1] - 1] - 2 * '0');
			s->integer[pos] = holder % 10 + '0';
			holder /= 10;
			l[0]--;
			l[1]--;
		}
